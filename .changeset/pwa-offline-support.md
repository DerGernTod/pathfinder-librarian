...
Viewed conversations are cached page-side (via the Cache API in `messages-store.fetchMessages`) so they remain accessible offline even before the service worker controls the page. The conversation list is cached the same way and invalidated on create/archive/restore/delete. Per-conversation messages use NetworkFirst in the SW so freshly submitted messages always render after reload; cache only kicks in when the network is unreachable.
